...
There is no package to install: the instructions further down install upstream's build, which has
no rate-limit segment. Build this one from source and point the statusline at the bundle.

```bash
git clone https://github.com/hurt-ai/claude-powerline.git
cd claude-powerline
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "bun /absolute/path/to/claude-powerline/dist/index.js --config=/absolute/path/to/claude-powerline.json"
  }
}
```

The statusline is a command run on every repaint, so editing the config takes effect on the next
one — nothing to restart. Editing the source does not: `dist/index.js` is what runs, so a source
change needs `bun run build` before it shows.
...
Branched from upstream in December 2025 and deliberately not tracking it since. This is not a
staging area for a pull request: what we need is here, upstream has none of it, and the two have
been diverging on purpose ever since. Everything below is upstream's own README, kept as it was
because everything it documents still works.
